...
#define ALLOCATE(VARIABLE, WORKSPACE, LENGTH, TT)   if (WORKSPACE == nullptr) {auto cres = cudaHostAlloc(reinterpret_cast<void **>(&VARIABLE), LENGTH * sizeof(TT), cudaHostAllocDefault); if (cres !=0) {nd4j_printf("CUDA host allocation failed, error code: %i\n", reinterpret_cast<int>(cres)); throw std::runtime_error("CUDA host allocation failed");};} else {VARIABLE = reinterpret_cast<TT*>(WORKSPACE->allocateBytes(LENGTH * sizeof(TT))); }
