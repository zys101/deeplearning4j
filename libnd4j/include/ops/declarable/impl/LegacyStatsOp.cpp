    ...
    namespace ops {
        ...
        Nd4jStatus LegacyStatsOp<T>::validateAndExecute(Context<T> &block) {
            ...
            if (block.getIArguments()->size() == 1 || (block.getIArguments()->size() == 2 && INT_ARG(1) == MAX_INT)) {
                // scalar
                LegacyOpExecutor<T>::execSummaryStatsScalarOp(*block.launchContext(), opNum, x, z, *block.getTArguments(), biasCorrected);
            } else {
                // dimensions for TAD
                ...
                LegacyOpExecutor<T>::execSummaryStatsOp(*block.launchContext(), opNum, x, z, dims, *block.getTArguments(),  biasCorrected);
            }
        ...
        */
        ...
        ShapeList *LegacyStatsOp<T>::calculateOutputShape(ShapeList *inputShape, nd4j::graph::Context<T> &block) {
            ...
            if (block.getIArguments()->size() == 0 || (block.getIArguments()->size() == 1 && INT_ARG(0) == MAX_INT)) {
                ...
                auto array = new NDArray<T>(nullptr, inShape, block.launchContext());
