...
JacobiSVD<T>::JacobiSVD(const NDArray<T>& matrix, const bool calcU, const bool calcV, const bool fullUV) {
    ...
    if(_calcU) {
        if(_fullUV)
            _u = NDArray<T>(matrix.ordering(), {_rows, _rows}, matrix.getContext());   
        else
            _u = NDArray<T>(matrix.ordering(), {_rows, _diagSize}, matrix.getContext());   
    }
    else 
        _u = NDArray<T>(matrix.ordering(), {_rows, 1}, matrix.getContext());   

    if(_calcV) {
        if(_fullUV)
            _v = NDArray<T>(matrix.ordering(), {_cols, _cols}, matrix.getContext());   
        else
            _v = NDArray<T>(matrix.ordering(), {_cols, _diagSize}, matrix.getContext());   
    }
    else 
        _v = NDArray<T>(matrix.ordering(), {_cols, 1}, matrix.getContext());   
    
    _m = NDArray<T>(matrix.ordering(), {_diagSize, _diagSize}, matrix.getContext());
...
void JacobiSVD<T>::mulRotationOnLeft(const int i, const int j, NDArray<T>& block, const NDArray<T>& rotation) {
    ...
    if(i < j) {
        ...
        NDArray<T> temp(block.ordering(), {2, block.sizeAt(1)}, block.getContext());
...
void JacobiSVD<T>::mulRotationOnRight(const int i, const int j, NDArray<T>& block, const NDArray<T>& rotation) {
    ...
    if(i < j) {
        ...
        NDArray<T> temp(block.ordering(), {block.sizeAt(0), 2}, block.getContext());
...
void JacobiSVD<T>::svd2x2(const NDArray<T>& block, int p, int q, NDArray<T>& left, NDArray<T>& right) {
    ...
    NDArray<T> m(block.ordering(), {2, 2}, block.getContext());
    m(0,0) = block(p,p);
    m(0,1) = block(p,q);
    m(1,0) = block(q,p);
    m(1,1) = block(q,q);
  
    NDArray<T> rotation(block.ordering(), {2, 2}, block.getContext());
    T t = m(0,0) + m(1,1);
    T d = m(1,0) - m(0,1);
        ...
        for(int p = 1; p < _diagSize; ++p) {
            ...
            for(int q = 0; q < p; ++q) {
                ...
                if(math::nd4j_abs<T>(_m(p,q)) > threshold || math::nd4j_abs<T>(_m(q,p)) > threshold){          
                    ...
                    {                                                                       
                        NDArray<T> rotLeft (_m.ordering(), {2, 2}, _m.getContext());
                        NDArray<T> rotRight(_m.ordering(), {2, 2}, _m.getContext());
